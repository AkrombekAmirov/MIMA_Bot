document.addEventListener("DOMContentLoaded", async () => {
  ...
  const userDirectionBox = document.getElementById("user-direction");
  ...
  try {
    ...
    userDirectionBox.innerHTML = `
      <div class="text-green-600 text-lg font-semibold mt-2">🎓 Mutaxassisligingiz: ${faculty}</div>
    `;

    // ✅ Tabriklash xabari (Yashil kayfiyatli)
    successMessage.classList.remove("hidden");

    // ✅ Umumiy ball
    totalScoreText.textContent = `${total_score} / 100`;

    // ✅ Har bir blok natijasi
    blocks.forEach(block => {
      ...
      card.innerHTML = `
        ...
        <h3 class="text-lg font-bold text-green-700 mb-2">🧩 ${block.block_number}-blok: ${block.subject_name}</h3>
        ...
        <div class="flex items-center justify-center mb-2">
          <div class="relative w-20 h-20">
            ...
            <div class="absolute top-0 left-0 w-full h-full flex items-center justify-center">
              <span class="text-md font-bold text-green-700">${percentage}%</span>
            </div>
          </div>
        </div>
