...
@dp.callback_query_handler(lambda call: call.data in ['info_law_1', 'info_law_2', 'info_law_3', 'info_law_4'])
async def subject(call: types.CallbackQuery, state: FSMContext):
    await db.add(FacultyBlock(block_number=1, subject_val='1', faculty_val='1'))
    await db.add(FacultyBlock(block_number=2, subject_val='2', faculty_val='1'))
    await db.add(FacultyBlock(block_number=3, subject_val='3', faculty_val='1'))
